...
//        }
    ...
    public boolean addOne(HOrder hOrder) {
        ...
        if (userId != null) {
            hOrder.setUserId(Convert.toLong(userId));
        } else {
            hOrder.setUserId(0L);
        }
    ...
    public boolean CancelOrder(Long id) {
        ...
        if (order != null) {
            // 检查订单是否属于当前登录用户
            if (StpUtil.isLogin()) {
                Long currentUserId = Long.valueOf(StpUtil.getLoginIdAsString());
...
//            room.setEndTime(hOrder.getEndDate());
    ...
    public boolean orderPlus(Long orderId, Integer days) {
        if (orderId == null || days == null || days <= 0) {
            return false;
        }
    ...
    public Page<HOrder> getUserOrders() {
        ...
        if (getLoginId != null) {
            Long loginId = Convert.toLong(getLoginId);
