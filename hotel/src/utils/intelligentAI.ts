...
(有任何需要随时召唤普普🍄)`
  ...
  private parseDates(dateText: string): { checkIn: string, checkOut: string } {
    ...
    for (const pattern of patterns) {
      ...
      if (match) {
        console.log('🎯 匹配到模式:', pattern.source, '结果:', match)
        ...
        if (pattern.source.includes('入住时间.*?离开时间')) {
          ...
          const checkInDay = match[1].padStart(2, '0')
          const checkOutDay = match[2].padStart(2, '0')
          return {
            checkIn: `${currentYear}-${currentMonth}-${checkInDay}`,
            checkOut: `${currentYear}-${currentMonth}-${checkOutDay}`
          }
